...
const PixelArtApp = ({ isDarkMode }: PixelArtAppProp) => {
  ...
  return (
    <div className={styles.wrapper}>
      <ColorPalette onSelectColor={setSelectedColor} selectedColor={selectedColor} />
      <PixelArtCanvas
        id={ID}
      ...
      />
      <Grid container sx={{ justifyContent: 'center', gap: '1rem', maxWidth: '80vw' }}>
        <Grid
          container
          sx={{
            gap: '2rem',
        ...
        >
          <IconButton aria-label="undo" size="small" color="default" onClick={handleUndo} className={styles.icon}>
            <FontAwesomeIcon
              icon={faArrowRotateLeft}
              color={`${isDarkMode ? 'var(--color-default)' : 'var(--color-mud)'}`}
            />
          </IconButton>
          <IconButton aria-label="undo" size="small" color="default" onClick={handleClear} className={styles.icon}>
            <FontAwesomeIcon icon={faTrashCan} color={`${isDarkMode ? 'var(--color-default)' : 'var(--color-mud)'}`} />
          </IconButton>
        </Grid>
        <Button
          variant="contained"
